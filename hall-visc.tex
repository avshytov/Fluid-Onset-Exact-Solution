...
\begin{align}
  \delta\dct{\rho}^\mathrm{(diff)}(k, q) = f_s F_{s, \rho}({\bm k}) = 
...
$\delta\dct{\Omega}({\bm k}) = f_s F_{s, \Omega}(k, q)$
with
\begin{align}
...
$\delta\dct{\rho}^{(I)} = I_0 F_{I, \rho}(k, q)$, with
\begin{align}
...
$\delta\dct{\Omega}^{(I)} = F_{I, \Omega}(k, q)$ with
\begin{align}
