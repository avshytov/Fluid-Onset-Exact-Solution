...
def do_fit(x, y, yv, fit_min, fit_max, xs, fit_func_re, fit_func_im, show_params = False, bounds=None):
    i_fit = [t for t in range(len(x)) if x[t] >= fit_min and x[t] <= fit_max]
    x_fit = np.array([x[t] for t in i_fit])
    y_fit = np.array([y[t] for t in i_fit])
    kwargs = {
...
def fit_append(k, y, Z, fit_min, fit_max, data_max, ymax, ks, fit_func_re, fit_func_im, func_scale, h = 0, bounds=None):
    #print Z.ndim
    do_flatten = False
    if Z.ndim == 1:
        Z = np.array([Z]).transpose()
...
def prepare_data(fname, sel, x, R_dir, **kwargs):
    ...
    if sel == '' or sel == 'iso':
      ...
      C_phi_j = 0.0 * C_rho
    if sel == 'cos':
      rho_k = drho_k = d['rho_cos'] * C_cos
      jx_k = d['jx_cos'] * C_cos
      jy_k = d['jy_cos'] * C_cos
      f_sk = d['f_cos']  * C_cos 
      phi_s = d['phi_s'] * C_cos
      phi_j = d['phi_cos'] * C_cos
      C_rho0 = 0.0 * C_cos
      C_phi_j = 0.0 * C_cos
    if sel == 'sin':
      rho_k = drho_k = d['rho_sin'] * C_sin
      jx_k = d['jx_sin'] * C_sin
      jy_k = d['jy_sin'] * C_sin
      f_sk = d['f_sin']  * C_sin 
      phi_s = d['phi_s'] * C_sin 
      phi_j = d['phi_sin'] * C_sin
      C_rho0 = 0.0 * C_sin
      C_phi_j = 0.0 * C_sin
    #bc_drho_j = d[drho_]
    ...
    k_joint, drho_k_joint = fit_prepend(k, y, drho_k, fit_small_min, fit_small_max,
