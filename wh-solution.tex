...
%\begin{equation}
%  \fminus{\chi}(k, q) = \frac{\gamma^2}{|k|(|k| + iq) K_\rho^\ast}
%\end{equation}
...
%(Note that~$\fminus{\psi}(k, q)$ cannot be normalised by
%the condition~$\fminus{\psi}(q = i|k|) %= 0$ due to a pole at this point.)
...
%  \ \ ,
%  \\
%  \psi^\ast(k) &= - \frac{k}{2\gamma} \frac{f_s(k)}{K_\Omega^\ast(k)}
  \ .
\end{align}
