...
\usepackage{bm,amssymb,amsfonts,amsmath,graphicx,hyperref,subfig,pgf}
...
\author{A.V.Shytov}
\affiliation{School of Physics, University of Exeter, EX4 4QL, United Kingdom}
\maketitle
...
\begin{figure}[h]
  ...
  %\inputpgf{data}{combined-phi-h=0.pgf}
  %\input{combined-phi-h=0.pdf_tex}
  ...
  \caption{
    ...
    %At large distances, the density of stream
...
\begin{figure}[h]
 \def\svgwidth{0.5\textwidth}
 \input{flux-vs-gamma-h=0.pdf_tex}
 %\inputpgf{data/}{flux-vs-gamma-h=0.pgf}
  \caption{
...
\begin{align}
  \tilde{\Phi}_s(k) =
  %- %\left(\frac{\pi} - 1 - \frac{2}{\pi}\right)
...
%
% Expansion for tilde{phi}/(1/pi - phi_s):
%     -(pi^2 - pi - 2)/pi + |k| - 0.2393 * k^2
%               -1.5049 + |k| - 0.2393 * k^2
% vs -(2*sqrt(2) - 1 - 2/pi)  + |k| - 0.7065 k^2  
%     1.192 
...
%        the slow-decaying pole contribution, while
...
\begin{align}
  ...
  % \frac{i \sgn k f_s(k)}{2\gamma'\gamma''}
  e^{-|k|y} \right. \\ \nonumber
  &\times \left.
       \left[
