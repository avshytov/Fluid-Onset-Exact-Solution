...
proportional to the induced density, we write
\begin{align}
...
\end{align}
The vorticity takes a similar form:
\begin{equation}
...
can be also rewritten as follows. The
contribution of divergence~$D_{\bm k}$ is simply isotropic
radial current emitted by the source. Such a flow is described
by the multivalued stream
function~$\Psi_\mathrm{rad} = \pi^{-1}\tan^{-1} \frac{y}{x}$.
...
\begin{align}
  ...
  \frac{1}{x^2}
\end{align}
if one assumes no-slip condition at the wall.
This comparison demonstrates that our solution faithfully
reproduces the hydrodynamical limit.
The next-order term, $-C_2 (k/\gamma)^2$ yields a contribution
$\propto x^{-3}$, which represents the lowest-order
correction to the viscous flow and can be attributed
to the finite slip length~$l_\mathrm{slip} = 2/\pi \gamma^{-1}$
(see the discussion in Sec.~\ref{sec:stokeslet}),
and other effects, e.g. the probe picking up higher harmonics
...
Let us begin with the emitter described by the source term $J^{\mathrm{edge}}(x, y, \alpha) = 2 I_0 \delta(x)\delta(y) \Theta(\alpha)$,
...
\end{equation}
represents the ``master integral'' through which we shall express all other one-sided contributions. A straightforward calculation yields
\begin{equation}
...
as per Eq.(\ref{eq:iso-xxx}), while the two terms on the
...
The direct-flight contribution to the vorticity is also straightforward, and it turns out to be $q$-indepdendent:
\begin{equation}
...
\ ,
\end{equation}
while~$\fplus{S}_\rho(k, q)$ is the same as in Eq.~(\ref{eq:chi-iso-xxx}),
so that the density~$\fminus{rho}(k, q)$ is changed by
...
$\Delta \Phi(k) = -I_0 / \gamma$, which coincides with the flux
...
the whole unit circle, which is decomposed into upward- and downward
...
the function~$\fplus{\rho}(k, q)$ is again given
by Eq.~(\ref{eq:chi-rho-xxx}). Similarly, $\fminus{\Omega}(k, q)$
...
\ .
\end{align}
Again, this expression simplifies to~$-1/\pi f_s(k)$. 
