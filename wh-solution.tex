...
\section{Analytical properties of the kernel and its factorisation}
...
\begin{align}
\fplus{K}_{\rho, \Omega}(k, \bar{q})
= \frac{1}{\fminus{\bar{K}}_{\rho, \Omega}(k, {\bar q})} \ ,
\fplus{K}_{\rho, \Omega}(\bar{q})
...
\begin{figure}
  \includegraphics[width=0.4\textwidth]{pole-and-cut-1.mps}
  \caption{
   \label{fig:contours}
...
In this model,
the  crossover `between the viscous and Ohmic behaviour occurs at the
length scale given
by~$l_\ast = \sqrt{\eta \sigma} / ne$. In the dimensionless units
employed throughout this paper,
$\eta = 1/(8 \gamma')$, $\sigma^{-1} = 1/2\gamma''$, and~$n = 1/2$, which
...
\section{Solving the Wiener-Hopf equations for generic sourcs}
...
can be drawn for generic sources.
...
Thus, one may write
\begin{align}
\delta\fplus{\rho}(k, q) = \rho_{\inf}(k, q) + \rho_\mathrm{reg}(k, q)\ ,
\qquad  \delta\rho_\mathrm{reg}(k, q) =
- \fplus{K}_\rho(k, q)
\fminus{S}_\rho(k, q) + \frac{\gamma'J_\mathrm{ext}^\ast}{|k|(|k| + iq)}
\frac{\fplus{K}_\rho(k, q)}{K_\rho^\ast(k)}\ .
\end{align}
The infinite-space solution can be found via the elementary approach.
Another representation of~$\fplus{\rho}$ can be obtained by noticing that
...
\end{align}
with the excess density~$\delta\rho_\infty(k, q)$ due to the
scattering in the infinite space given by
\begin{align}
\delta\rho_\infty(k, q) = \left[\frac{1}{K_\rho(k, q)} - 1\right]
\dct{\rho}(k, q)\ .
- \frac{2\gamma' J_\mathrm{ext}(k, q)}{k^2 + q^2}
\end{align}
The advantage of using this representation is that the real-space
...
\begin{align}
  \fplus{\Omega}_{\bm k} = \fplus{K}_\Omega(k, q) (\fplus{S}_\Omega(k, q)
  + C_\Omega)
  \ ,
  \\
  \fminus{\Omega}_{\bm k} = \fminus{K}_\Omega(k, q) (\fminus{S}_\Omega(k, q)
  - C_\Omega)
  \ ,
\end{align}
...
\begin{align}
  ...
  \fplus{\Omega}_{\bm k} = \dct{Omega}(k, q) + \Omega_\mathrm{req}(k, q)\ , \\
  \Omega_\mathrm{reg}(k, q) =
  \left(\frac{1}{K_\Omega(k, q)} - 1 \right) \Omega_\mathrm{dct}(k, q)
  ...
  \ .
\end{align}
Here we absorbed the constant~$S_\Omega^\ast$
into the function~$\fminus{S}_\Omega$ by introducing
${\tilde S}_\Omega(k, q) = \fminus{S}_\Omega(k, q) + S_\omega^\ast$.
...
\begin{align}
  ...
  \ .
\end{align}
This integral is convertent even for~$\dct{\Omega}$ tending to a non-zero limit at~$q'\to \infty$.
...
\int_{-\pi}^{0} \frac{\fminus{J}_k(\alpha)}{\gamma - i {\bm k}\cdot{\bm v}}
\frac{d\alpha}{2\pi}\ ,
\qquad
\Delta \dct{\fminus{\rho}}({\bm k}) =
\int_{-\pi}^{0} \frac{{\bm k}\times{\bm v}\fminus{J}_k(\alpha)}{
...
one finds:
\begin{align}
\dct{\rho}^{(diff)}(k, q) = \frac{iq}{k^2 + q^2} K_\rho(k, q)\ ,
\qquad
\dct{\Omega}^{diff}(k, q) = \frac{k}{2\gamma} \left[1 - K_\Omega(k, q)\right]
\end{align}
...
and~(\ref{eq:source-surf-omega}). In this case, $\fminus{S}_\rho(k, q)$ can be
...
and~$\fminus{S}_\Omega(k, q)$ similarly to Eq.(\ref{eq:phi-chi-psi}):
\begin{equation}
...
%  = I_0 \left\{- \frac{2\gamma}{|k|}
%    + \frac{1 - 2/K_\rho^\ast(k)}{
%           \gamma/(4|k|)/\left[k_\rho^\ast(k)\right]^2}
%           + \frac{k}{2\gamma} \left\{1/K_\Omega^\ast(k)\right]^2 - 1\right\}
%        }\right\}
  \ ,
\end{equation}
