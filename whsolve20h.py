...
def wh_solve(h, delta, k, yv, src='isotropic'):
    ...
    phi_f =  1.0/np.pi - 2 * 0.25 * k/gamma  - 2 * 0.125 * gamma/k/Xrho_0**2 
    phi_f += 2 * 0.25 * k/gamma * 1/Xo_0**2
    print("phi_f = ", phi_f)
    print("phi_f - 1/pi = ", phi_f - 1.0/np.pi)

    # Bulk and surface current sources require separate treatment
    if (h > delta):
        phi_j  =  - 2 * gamma**2 / 4.0 / absk**2 / Xrho_0**2 * J_0
        phi_j +=  - 0 * gamma / np.pi / k * J_0
        phi_j += 0 * gamma**2 / k * Frho_0 * J_0
        phi_j += 2 * ( 0.5 / Xo_0**2  + 0 * gamma/k * Fomega_0) * J_0
        phi_chi  = c_down_int(np.exp(-1j*z_down*h)/(z_down - 1j * absk) \
                              *(1.0/Xrho_m_down))
        phi_chi /=  2.0 * np.pi * 1j * Xrho_0 * 2.0 * 0.5 
        phi_j += phi_chi
        print("phi_j = ", phi_j)
        print("phi_chi = ", phi_chi)
    else:
        phi_j =  2 * (0.5  -  1.0/Xrho_0 - 0.25 * gamma**2 / k**2 / Xrho_0**2)
        phi_j += 2 * 0.5/Xo_0**2
    print("flux phi_j", phi_j)

    exp_kh = np.exp(-absk * h)

    flux_sin_cos_comm   = - 4 * 0.125 * gamma / absk / Xrho_0**2 * exp_kh
    flux_sin_cos_comm += 4 * 0.25 * k / gamma / Xo_0**2 * exp_kh
    dflux_sin = - 4 * 0.25/np.pi/gamma/Xo_0 * k \
            *  c_down_int(k/(z_down**2 + k**2)/Xomega_m_down \
                          * np.exp(-1j*z_down*h))
    dflux_cos = - 4 * 0.25/np.pi/gamma/Xo_0 * k \
    ...
    rho_sin_up  = -2 * 1j * z_up/(z_up**2 + k**2) * exp_zyh_up_sgn
    rho_sin_up += 2 * 0.5 / (absk - 1j * z_up)*exp_kh/Xrho_0 * Xrho_p_up * exp_zy_up
    rho_cos_up  = - 2 * 1j * k/(z_up**2 + k**2) * exp_zyh_up
    rho_cos_up += 2 * 0.5  * 1j * sgn_k / (absk - 1j * z_up)*exp_kh/Xrho_0 * Xrho_p_up * exp_zy_up
    ...
    jx_cos_b = 2 * 0.5/gamma * c_up_int((-z_up) * k/(z_up**2 + k**2)*(1.0 - 1.0/Xo_0) * Xomega_p_up * exp_zy_up) * exp_kh / 2.0 / np.pi * 1j * sgn_k
    jx_sin_b = 2 * 0.5/gamma * c_up_int(k * (-z_up)/(z_up**2 + k**2)*(1.0 - 1.0/Xo_0) * Xomega_p_up*exp_zy_up) * exp_kh / 2.0 / np.pi 
    jy_cos_b = 2 * 0.5/gamma * c_up_int(k * k/(z_up**2 + k**2)*(1.0 - 1.0/Xo_0) * Xomega_p_up*exp_zy_up) * exp_kh / 2.0 / np.pi * 1j * sgn_k
    jy_sin_b = 2 * 0.5/gamma * c_up_int(k * k/(z_up**2 + k**2)*(1.0 - 1.0/Xo_0) * Xomega_p_up * exp_zy_up) * exp_kh  / 2.0 / np.pi 
    ...
    jx_cos_d = 2 * c_up_int(1.0/(z_up - 1j * k) * (-z_up) * Jpsi_cos_up * Xomega_p_up * exp_zy_up) / 2.0 / np.pi * 0.5 / gamma
    jx_sin_d = 2 * c_up_int(1.0/(z_up - 1j * k) * (-z_up) * Jpsi_sin_up * Xomega_p_up * exp_zy_up) / 2.0 / np.pi * 0.5 / gamma
    jy_cos_d = 2 * c_up_int(1.0/(z_up - 1j * k) * (k) * Jpsi_cos_up * Xomega_p_up * exp_zy_up) / 2.0 / np.pi * 0.5 / gamma
    jy_sin_d = 2 * c_up_int(1.0/(z_up - 1j * k) * (k) * Jpsi_sin_up * Xomega_p_up * exp_zy_up) / 2.0 / np.pi * 0.5 / gamma
...
if __name__ == '__main__':
    ...
    if True:
       ...
       ###kvals = list(np.linspace(0.001, 0.009, 5))
    ...
    def interim_save():
        np.savez("wh-data20-h=%g-delta=%g-kmin=%g-kmax=%g" % (h, delta,
                 ...
                 rho_s=result.rho_s,
                 bare_rho_j=result.rho_j,
                 bare_drho_j=result.drho_j,
                 bare_jx=result.jx_j,
                 bare_jy=result.jy_j,
                 bare_rho_cos = result.rho_cos,
                 bare_rho_sin = result.rho_sin,
                 bare_jx_cos = result.jx_cos,
                 bare_jy_cos = result.jy_cos,
                 bare_jx_sin = result.jx_sin,
                 bare_jy_sin = result.jy_sin,
                 jx_s = result.jx_s, jy_s = result.jy_s,
